...
namespace Picoware
{
    ...
    bool WiFiUtils::setTime(tm &timeinfo, int timeoutMs)
    {
        ...
        if (!ntpInitialized)
        {
            // ESP32 SNTP: UTC (no offset), poll pool.ntp.org / time.nist.gov.
            configTime(0, 0, "pool.ntp.org", "time.nist.gov");
            ntpInitialized = true;
        }
        // Waits up to timeoutMs for the system time to be set by SNTP.
        if (!getLocalTime(&timeinfo, (uint32_t)timeoutMs))
            return false;
